...
impl CdpClient {
    ...
    pub async fn set_extra_headers(&self, headers: &HashMap<String, String>) -> Result<()> {
        self.send_command("Network.setExtraHTTPHeaders", json!({ "headers": headers })).await?;
        Ok(())
    }

    pub async fn block_urls(&self, patterns: &[&str]) -> Result<()> {
        self.send_command("Network.setBlockedURLs", json!({ "urls": patterns })).await?;
        Ok(())
    }
    ...
    pub async fn intercept_requests(&self, url_patterns: &[&str]) -> Result<()> {
        ...
        let patterns: Vec<_> = url_patterns
            ...
            .collect();
        self.send_command("Fetch.enable", json!({ "patterns": patterns })).await?;
        Ok(())
    }

    pub async fn continue_request(&self, request_id: &str) -> Result<()> {
        self.send_command("Fetch.continueRequest", json!({ "requestId": request_id })).await?;
        Ok(())
    }
    ...
    pub async fn fulfill_request(
        ...
        self.send_command("Fetch.fulfillRequest", json!({
