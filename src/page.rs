...
impl CdpClient {
    pub async fn set_content(&self, html: &str) -> Result<()> {
        ...
        self.send_command("Page.setDocumentContent", json!({
            "frameId": frame_id,
            "html":    html,
        })).await?;
        Ok(())
    ...
    pub async fn print_to_pdf(&self, path: &str) -> Result<()> {
        let result = self.send_command("Page.printToPDF", json!({
    ...
    pub async fn add_init_script(&self, source: &str) -> Result<String> {
        let result = self
            .send_command("Page.addScriptToEvaluateOnNewDocument", json!({ "source": source }))
            .await?;
    ...
    pub async fn set_user_agent(&self, ua: &str) -> Result<()> {
        self.send_command("Emulation.setUserAgentOverride", json!({ "userAgent": ua })).await?;
        Ok(())
    }
    ...
    pub async fn set_offline(&self, offline: bool) -> Result<()> {
        self.send_command("Network.emulateNetworkConditions", json!({
            "offline":            offline,
            "latency":            0,
    ...
    pub async fn set_attribute(&self, node_id: i64, name: &str, value: &str) -> Result<()> {
        self.send_command("DOM.setAttributeValue", json!({
            "nodeId": node_id,
            "name":   name,
            "value":  value,
        })).await?;
    ...
    pub async fn set_outer_html(&self, node_id: i64, html: &str) -> Result<()> {
        self.send_command("DOM.setOuterHTML", json!({
            "nodeId":    node_id,
            "outerHTML": html,
        })).await?;
        Ok(())
    ...
    pub async fn remove_node(&self, node_id: i64) -> Result<()> {
        self.send_command("DOM.removeNode", json!({ "nodeId": node_id })).await?;
        Ok(())
    }
    ...
    pub async fn call_function_on(
        ...
        let result = self
            .send_command("Runtime.callFunctionOn", json!({
                "objectId":            object_id,
